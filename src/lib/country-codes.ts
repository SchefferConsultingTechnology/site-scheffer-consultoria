...
export type CountryCode = { iso2: string; dialCode: string };

// ISO 3166-1 alpha-2 code + ITU-T E.164 country calling code. Static reference data (not
// content), so it isn't translated — display names are derived at render time via
// `countryName()` instead of being hardcoded per language.
export const COUNTRY_CODES: CountryCode[] = [
  ...
  { iso2: "BL", dialCode: "590" },
  { iso2: "BM", dialCode: "1" },
  { iso2: "BN", dialCode: "673" },
  { iso2: "BO", dialCode: "591" },
  { iso2: "BQ", dialCode: "599" },
  { iso2: "BR", dialCode: "55" },
  { iso2: "BS", dialCode: "1" },
  ...
  { iso2: "YT", dialCode: "262" },
  { iso2: "ZA", dialCode: "27" },
  { iso2: "ZM", dialCode: "260" },
  { iso2: "ZW", dialCode: "263" },
];

export const DEFAULT_COUNTRY_ISO2: Record<Locale, string> = {
  pt: "BR",
  en: "US",
  es: "ES",
};
...
// Converts an ISO 3166-1 alpha-2 code into its flag emoji by mapping each letter to a Unicode
// regional indicator symbol (e.g. "BR" -> 🇧 + 🇷 -> 🇧🇷) — no per-country data needed.
...
// Localized country name via the browser/runtime's built-in Intl.DisplayNames — avoids hand-
// translating ~200 country names into pt/en/es.
export function countryName(iso2: string, locale: Locale) {
  try {
    return new Intl.DisplayNames([INTL_LOCALE[locale]], { type: "region" }).of(iso2) ?? iso2;
  } catch {
    return iso2;
  }
}
